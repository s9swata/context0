...
var NullValueError = new ContractError(
	"Value cant be null, use remove instead.",
);
var NotOwnerError = new ContractError("Not contract owner.");
var InvalidFunctionError = new ContractError("Invalid function.");
var ArrayLengthMismatchError = new ContractError(
	"Key and value counts mismatch.",
);
...
var verifyProof = async (proof, psignals, verificationKey) => {
	...
	return await SmartWeave.extensions[verificationKey.protocol].verify(
		verificationKey,
		psignals,
		proof,
	);
...
var hashToGroup = (value) => {
	if (value) {
		return BigInt(
			SmartWeave.extensions.ethers.utils.ripemd160(
	...
	switch (input.function) {
		...
		case "getMany": {
			...
			const values = await Promise.all(
				keys.map((key) => SmartWeave.kv.get(key)),
			);
		...
		case "setMany": {
			...
			await Promise.all(
				keys.map((key, i) => SmartWeave.kv.put(key, values[i])),
			);
			return { state };
		}
		case "getKeys": {
		...
		case "putMany": {
			...
			if (
				await Promise.all(keys.map((key) => SmartWeave.kv.get(key))).then(
					(values2) => values2.some((val) => val !== null),
				)
			) {
				throw KeyExistsError;
			}
		...
		case "update": {
			const { key, value } = await apply(
				...
				onlyProofVerified("auth", async (_, input2) => {
					...
					return [
						hashToGroup(oldValue),
						hashToGroup(input2.value),
						BigInt(input2.key),
					];
				}),
			);
		...
		case "updateVerificationKey": {
			const { name, verificationKey } = await apply(
				caller,
				input.value,
				state,
				onlyOwner,
		...
		case "updateWhitelist": {
			const { add, remove, name } = await apply(
				caller,
				input.value,
				state,
				onlyOwner,
